...
public class EquivalenceProof implements Proof {
    ...
    public void applyNewUserCommand(String uCommand) throws InvalidRuleApplicationException {
        ...
        try {
            ...
            uc.setProof(this);
            try {
                if (!(uc instanceof UndoCommand)) _history.add(new ProofHistory((IProofState) _ps, uc, _bottom, _lcTrs));
                _ps = uc.apply(new ProofState(_ps));
                updateVariables();
    ...
    public UserCommand getLastCommand() {
        if (_history.size() <= 0) throw new IndexOutOfBoundsException("History is empty");
